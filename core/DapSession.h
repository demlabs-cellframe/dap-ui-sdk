...
protected:
    ...
    DapNetworkReply* encRequest(const QString& reqData, const QString& url, const QString& subUrl,
                               const QString& query, QObject* obj, const char* slot, const char* slot_err, bool isCDB, bool isСriticalReq = true);
    ...
    DapNetworkReply* encRequest(const QString& reqData,const QString& url,
                                const QString& subUrl,const QString& query, bool isCDB, bool isСriticalReq = true) {
        return encRequest(reqData, url, subUrl, query, this, NULL, NULL, isCDB, isСriticalReq);
    }

    DapNetworkReply* encRequest(const QString& reqData, const QString& url,
                    const QString& subUrl, const QString& query, const char* slot, const char* slot_err, bool isCDB = false, bool isСriticalReq = true)
    {
        return encRequest(reqData, url, subUrl, query, this, slot, slot_err, isCDB, isСriticalReq);
    }
...
private:
    ...
    DapNetworkReply* _buildNetworkReplyReq(const QString& urlPath, QObject *obj, const char *slot, const char *slot_err,
                                         const QByteArray* data = Q_NULLPTR, bool isCDB = false, bool isСriticalReq = true/*, DapNetworkReply *netReply = nullptr*/);
